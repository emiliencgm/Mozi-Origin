...
class Graph_Construct:
    ...
    def __init__(self, side_self, args, side_ops=None):
        ...
        #加载谷歌预训练的Universal Sentence Encoder模型（目前对中文句子的嵌入并不友好）可以使用此对str类型的属性嵌入为高维向量
        # self.sentence_encoder = hub.load("graph_representation/NLP_Sentence_Embedding_Model")
    ...
    def get_summary_entity_attr(self, entity):
        ...
        for i in range(self.attr_dim_list[0]):
            ...
            if self.attr_vector_list_all_1[i] in ['side', 'ClassName', 'strName', 'strActiveUnitStatus', 'strDamageState']:
                ...
                # self.parse_attr(self.attr_vector_list_all_1[i], entity, self.vector)
            else:                
                #值本身就为float类型，直接使用
                vector[self.j] = entity_attr_dict[self.attr_vector_list_all_1[i]]
                # self.vector.append(entity_attr_dict[self.attr_vector_list_all_1[i]])
        ...
        for i in range(self.attr_dim_list[1]):
            ...
            # self.parse_attr2(self.attr_vector_list_all_2[i], entity, self.vector)
            self.j += 1
        ...
        for i in range(self.attr_dim_list[2]):
            if self.attr_vector_list_aircrafts[i] in ['strFuelState']:
                ...
                # self.parse_attr(self.attr_vector_list_aircrafts[i], entity, self.vector)
            else:
                try:
                    vector[self.j] = entity_attr_dict[self.attr_vector_list_aircrafts[i]]
                    # self.vector.append(entity_attr_dict[self.attr_vector_list_aircrafts[i]])
                except:
                    pass
            self.j += 1
        ...
        for i in range(self.attr_dim_list[3]):
            if self.attr_vector_list_facilities[i] in ['']:
                ...
                # self.parse_attr(self.attr_vector_list_facilities[i], entity, self.vector)
            else:
                try:
                    vector[self.j] = entity_attr_dict[self.attr_vector_list_facilities[i]]
                    # self.vector.append(entity_attr_dict[self.attr_vector_list_facilities[i]])
                except:
                    pass
            self.j += 1
        #舰船专有的属性
        for i in range(self.attr_dim_list[4]):
            if self.attr_vector_list_ships[i] in ['']:
                ...
                # self.parse_attr(self.attr_vector_list_ships[i], entity, self.vector)
            else:
                try:
                    vector[self.j] = entity_attr_dict[self.attr_vector_list_ships[i]]
                    # self.vector.append(entity_attr_dict[self.attr_vector_list_ships[i]])
                except:
                    pass
            self.j += 1
        #潜艇专有的属性
        for i in range(self.attr_dim_list[5]):
            if self.attr_vector_list_submarines[i] in ['']:
                ...
                # self.parse_attr(self.attr_vector_list_submarines[i], entity, self.vector)
            else:
                try:
                    vector[self.j] = entity_attr_dict[self.attr_vector_list_submarines[i]]
                    # self.vector.append(entity_attr_dict[self.attr_vector_list_submarines[i]])
                except:
                    pass
            self.j += 1
        #武器专有的属性
        for i in range(self.attr_dim_list[6]):
            if self.attr_vector_list_weapons[i] in ['']:
                ...
                # self.parse_attr(self.attr_vector_list_weapons[i], entity, self.vector)
            else:
                try:
                    vector[self.j] = entity_attr_dict[self.attr_vector_list_weapons[i]]
                    # self.vector.append(entity_attr_dict[self.attr_vector_list_weapons[i]])
                except:
                    pass
            self.j += 1
        #卫星专有的属性
        for i in range(self.attr_dim_list[7]):
            if self.attr_vector_list_satellites[i] in ['']:
                ...
                # self.parse_attr(self.attr_vector_list_satellites[i], entity, self.vector)
            else:
                try:
                    vector[self.j] = entity_attr_dict[self.attr_vector_list_satellites[i]]
                    # self.vector.append(entity_attr_dict[self.attr_vector_list_satellites[i]])
                except:
                    pass
            self.j += 1
    ...
    def parse_strActiveUnitStatus_attr(self, attr_name, entity):
        ...
        # emb_status = self.sentence_encoder([entity.__dict__[attr_name]])[0]
        # for i in emb_status:
