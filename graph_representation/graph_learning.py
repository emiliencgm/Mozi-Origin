...
class Graph_Learning:
    ...
    def train(self):
        ...
        if self.count % self.args.visual_step == 0:
            num_blue = self.graph_construct.num_nodes_self
            self.graph_visual.t_SNE(torch.tensor(self.node_attr), num_blue, 'ego')
            self.graph_visual.t_SNE(self.encoder(data), num_blue, 'emb')#.clone().detach()
