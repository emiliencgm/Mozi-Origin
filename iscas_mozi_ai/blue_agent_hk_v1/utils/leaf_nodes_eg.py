...
def create_antisurfaceship_mission(side_name, scenario):
    ...
    # 获取反舰任务飞机
    # 闪电#1 #2
    airs_1 = {k: v for k, v in airs_dic.items() if int(
        re.sub('\D', '', v.strName)) < 3}
    # 闪电#3 #4
    airs_2 = {k: v for k, v in airs_dic.items() if 3 <= int(
        re.sub('\D', '', v.strName)) <= 4}
    ...
    targets = {k: v for k, v in contacts.items() if (
        ('DDG' in v.strName) | ('CVN' in v.strName))}
...
def update_antisurfaceship_mission(side_name, scenario):
    ...
    airs = {k: v for k, v in airs_dic.items() if int(
        re.sub('\D', '', v.strName)) < 9}
    contacts = side.contacts
...
def update_patrol_mission(side_name, scenario):
    ...
    for patrol_mission in patrol_missions:
        if patrol_mission.strName == side_name + 'xl1':
            ...
            airs_xl1 = {k: airs[k]
                        for k, v in airs.items() if v.strName in lst[:3]}
            # 分配单元
        ...
        if patrol_mission.strName == side_name + 'xl2':
            ...
            airs_xl2 = {k: airs[k]
                        for k, v in airs.items() if v.strName in lst[3:6]}
        ...
        if patrol_mission.strName == side_name + 'xl3':
            ...
            airs_xl3 = {k: airs[k]
                        for k, v in airs.items() if v.strName in lst[6:8]}
            patrol_mission.assign_units(airs_xl3)
            patrol_mission.set_is_active('false')

            # 如果飞机飞出，任务就打开
            airs3 = [v for v in airs_xl3.values(
            ) if '在空' in v.strActiveUnitStatus]
            if len(airs3):
            ...
            list_str = [
                air.m_AssignedMission for air in air_other if '停放' in air.strActiveUnitStatus]
            list_str = [i for i in list_str if i != '']
            set_str = set(list_str)
            air_sky = [
                air for air in air_other if '按计划' in air.strActiveUnitStatus or '参与' in air.strActiveUnitStatus]
        ...
        if patrol_mission.strName == side_name + 'xl4':
            ...
            airs_xl4 = {k: airs[k]
                        for k, v in airs.items() if v.strName in lst[8:]}
...
def evade_ship(geopoint_target, air, mission_doctrine):
    ...
    if geopoint_target:
        ...
        if dis <= 60:
            ...
            genpoint_away = get_end_point(
                geopoint_air, 15, (air.fCurrentHeading + 150))
...
def create_patrol_mission(side_name, scenario):
    ...
    if flag == 'Yes':
        return False
    airs_c = [v for v in airs_dic.values() if 4 < int(
        re.sub('\D', '', v.strName)) < 9]  # 编号5，6,7,8的,4架飞机
    for air in airs_c:
        # 如果此4架飞机的挂载还是原来的挂载，那么修改挂载id 为 19364
        if 'AGM' in air.m_UnitWeapons:
            air.set_loadout('19364', time_to_ready_minutes=1,
                            ignore_magazines='true', exclude_optional_weapons='false')
    # patrol_missions_dic = side.get_patrol_missions()
    patrol_mission_name = [
...
def create_patrol_zone(side_name, scenario):
    ...
    for k, v in ships.items():
        ...
        if '驱逐舰' in v.strName:
            ...
            # xl1
            rp1 = side.add_reference_point(
                side_name + 'rp1', lat + 1.2, lon + 1)
            rp2 = side.add_reference_point(
                side_name + 'rp2', lat + 1, lon + 1.5)
            rp3 = side.add_reference_point(
                side_name + 'rp3', lat + 0.6, lon + 1)
            rp4 = side.add_reference_point(
                side_name + 'rp4', lat + 1, lon + 0.5)
            point_list.append([rp1, rp2, rp3, rp4])
            # xl2
            rp5 = side.add_reference_point(
                side_name + 'rp5', lat - 0.7, lon + 1)
            rp6 = side.add_reference_point(
                side_name + 'rp6', lat - 1, lon + 1.5)
            rp7 = side.add_reference_point(
                side_name + 'rp7', lat - 1.2, lon + 1)
            rp8 = side.add_reference_point(
                side_name + 'rp8', lat - 1, lon + 0.6)
            point_list.append([rp5, rp6, rp7, rp8])
            # xl3
            rp13 = side.add_reference_point(
                side_name + 'rp13', lat + 0.25, lon - 1)
            rp14 = side.add_reference_point(
                side_name + 'rp14', lat + 0.25, lon - 0.3)
            rp15 = side.add_reference_point(
                side_name + 'rp15', lat - 0.25, lon - 0.3)
            rp16 = side.add_reference_point(
                side_name + 'rp16', lat - 0.25, lon - 1)
    ...
    for contact in contacts.values():
        if '驱逐舰' in contact.strName:
            ...
            rp9 = side.add_reference_point(
                side_name + 'rp9', lat1 + 0.7, lon1 - 0.5)
            rp10 = side.add_reference_point(
                side_name + 'rp10', lat1 + 0.7, lon1 + 0.5)
            rp11 = side.add_reference_point(
                side_name + 'rp11', lat1 - 0.7, lon1 + 0.5)
            rp12 = side.add_reference_point(
                side_name + 'rp12', lat1 - 0.7, lon1 - 0.5)
