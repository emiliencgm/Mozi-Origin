...
功能：
'''
class feihaiblue_all(BaseEnvironment):
    """
    作者：
    日期：
    功能：构造函数
    ...
    def __init__(self, IP, AIPort, agent_key_event_file, duration_interval, app_mode, synchronous=None,
        ...
        self.action_space_ship = 2  # 连续型动作[x,x]
        # self.action_max = 8
    ...
    def reset(self):
        # 调用父类的重置函数
        super(feihaiblue_all, self).reset()
    ...
    def execute_action(self, action):
        super(feihaiblue_all, self).step()
        nine_zone = self.train_zones
        print('action=', action)
    ...
    def execute_action_ship(self, action):
        ...
        reward_ship = self.get_reward_ship(action)
        done_ship = self.check_done()
    ...
    def create_strike_ship_mission_ship(self, side):
        ...
        if {k: v for k, v in mssnSitu.items() if v.strName == 'strike1'}.__len__() == 0:
            ...
            strike_target = {key:value for key,value in self.red_unit_dict_ship.items() if '护卫舰' in value.strName}
            ...
            strkmssn_1.assign_units(self.blue_unit_dict_ship)
            strkmssn_1.add_plan_way_to_mission(0, '航线2')
            return True
        else:
            return False
    def set_plot_point_ship(self,action):
    ...
    def get_reward_ship(self, action):
        """
        ...
        if len(action) != 0:
            if abs(action[0]) < 0.2 and abs(action[1]) < 0.3:
                ...
                # 对三个reward 进行加权求和作为总reward
                reward = 0.5 * distance_reward_1 + 0.1 * score_reward + 0.4 * distance_reward_2
            else:
                reward -= 0.5
        return reward
