...
from blue_agent_lj.feihai_blue_ppo_test_v3.envs.utils import *
...
功能：
'''
class feihaiblue_all_v3(BaseEnvironment):
    """
    作者：
    日期：
    功能：构造函数
    ...
    def __init__(self, IP, AIPort, agent_key_event_file, duration_interval, app_mode, synchronous=None,
        ...
        self.action_space_ship = 9  # 离散型动作，9宫格
        # self.action_max = 8
    ...
    def _init_red_unit_dict(self):
        """
        初始化红方单元列表，在测试阶段，改成读contacts，从蓝方视角
        """
        ...
        # red_aircraft_dict = {aircraft_key: aircraft_value for aircraft_key, aircraft_value in
        #                      self.blueside.contacts.items() if
        #                      '米格' in aircraft_value.strName}
        print('red_aircraft_dict=', red_aircraft_dict)
    ...
    def reset(self):
        # 调用父类的重置函数
        super(feihaiblue_all_v3, self).reset()

        # 构建各方实体
        self._construct_side_entity()
        self._init_unit_list()
        self._init_unit_list_ship()
        # 构建9宫格
        ...
        state_now_ship = self.get_observation_ship()
        return state_now, state_now_ship
    '''
    def reset_ppo(self):
        # 构建各方实体
        self._construct_side_entity()
    ...
    def execute_action(self, action):
        super(feihaiblue_all_v3, self).step()
        nine_zone = self.train_zones
        print('action=', action)
    ...
    def _init_red_unit_dict_ship(self):
        """
        初始化红方单元列表,测试阶段，改成从蓝方视角读contacts
        """
        ...
        # red_ship_dict = {ship_key: ship_value for ship_key, ship_value in self.blueside.contacts.items() if
        #                  '护卫舰' in ship_value.strName or '驱逐舰' in ship_value.strName or '航空母舰' in ship_value.strName}
    ...
    def get_observation_ship(self):
        ...
        """
        blue_aircraft_unit_dict_ship = self.blue_unit_dict_ship
        blue_obs_ship = self.get_blue_side_observation_ship(blue_aircraft_unit_dict_ship)
        red_ship_unit_dict_ship = self.red_unit_dict_ship
        red_obs_ship = self.get_red_side_observation_ship(red_ship_unit_dict_ship)
        all_obs_array_ship = np.array(blue_obs_ship + red_obs_ship)
        self.observation_ship = all_obs_array_ship
        return all_obs_array_ship
    ...
    def execute_action_ship(self, action):
        # 首先下达一个反水面巡逻，然后更新巡逻区
        nine_zone = self.train_zones_ship
        print('action_ship=', action)
        final_patrol_zone = nine_zone[action]

        # flag = self.create_strike_ship_mission_ship(self.blueside)
        flag = self.init_creat_patrol_ship_mission()
        if flag:
            # 如果已经有打击任务了，就不在创建了，直接利用学习到action设置航路点

            # self.set_plot_point_ship(action)
        ...
        obs_ship = self.get_observation_ship()
        reward_ship = self.get_reward_ship(final_patrol_zone)
        done_ship = self.check_done_ship()

        return np.array(obs_ship), reward_ship, done_ship
