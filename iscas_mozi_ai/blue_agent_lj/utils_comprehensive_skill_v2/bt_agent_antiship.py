...
from blue_agent_lj.utils_comprehensive_skill_v2.leaf_nodes_eg import *
...
class CAgent:
    ...
    def init_bt(self, env, side_name, lenAI, options):
        ...
        missionSelectorCondition.set_action(antiship_condition_check, sideGuid,
                                            shortSideKey, attributes)

        patrolMissionSelector.set_action(patrolMissionSelector.select,
                                         sideGuid, shortSideKey, attributes)

        updatePatrolMission.set_action(update_patrol_mission, sideGuid,
                                       shortSideKey, attributes)
        createPatrolMission.set_action(create_patrol_mission, sideGuid,
                                       shortSideKey, attributes)

        AntiSurfaceShipMissionSelector.set_action(
            AntiSurfaceShipMissionSelector.select, sideGuid, shortSideKey,
            attributes)
        CreateAntiSurfaceShipMission.set_action(create_antisurfaceship_mission,
                                                sideGuid, shortSideKey,
                                                attributes)
        UpdateAntiSurfaceShipMission.set_action(update_antisurfaceship_mission,
                                                sideGuid, shortSideKey,
                                                attributes)
        self.bt = hxSequence
