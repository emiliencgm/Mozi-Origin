...
def create_antisurfaceship_mission(side_name, scenario):
    ...
    strike_patrol_missions_2 = [v for v in patrol_missions_dic.values() if 'strike_patrol_2' in v.strName]
    ...
    if len(airs_fight_red) <= 5:
        update_patrol_zone(side_name, scenario)
        update_patrol_zone_2(side_name, scenario)
        update_patrol_zone_3(side_name, scenario)
        update_patrol_zone_4(side_name, scenario)
    if len(airs_fight_red) <= 2:
        update_patrol_zone_1_2(side_name, scenario)
        update_patrol_zone_2_2(side_name, scenario)
        update_patrol_zone_3_2(side_name, scenario)
        update_patrol_zone_4_2(side_name, scenario)
        update_support_zone_1(side_name, scenario)

    support_point = [20.8612760556798, 122.97096027272]

    airs_yjj = {k: v for k, v in airs_dic.items() if v.strName in lst_5}
    unit_yjj = []
    for k, v in airs_yjj.items():
        unit_yjj.append(v.dLatitude)
        unit_yjj.append(v.dLongitude)
    dis_temp1 = get_horizontal_distance(support_point, unit_yjj)
    # 如果预警机和干扰机1到达新区域，将巡逻任务2，3，4换成反水面巡逻2，3，4
    if dis_temp1 < 8:
        ...
        if len(strike_patrol_missions_2) == 0:
            print('开始创建反水面巡逻任务')
...
def update_antisurfaceship_mission(side_name, scenario):
    ...
    for patrol_mission in strike_patrol_missions:
        if patrol_mission.strName == 'strike_patrol_2':
            patrol_mission.set_prosecution_zone(ps_anti_str)
    ...
    for patrol_mission in patrol_missions:
        if patrol_mission.strName == side_name + 'xl1':
            ...
            for air in airs_xl2.values():
                ...
                doctrine.set_emcon_according_to_superiors('no')
                doctrine.set_em_control_status('Radar', 'Passive')
        elif patrol_mission.strName == side_name + 'xl3':
            patrol_mission.set_prosecution_zone(ps_str_3)
            doctrine_xl3 = patrol_mission.get_doctrine()
            doctrine_xl3.set_weapon_state_for_aircraft(2002)
            doctrine_xl3.set_weapon_state_for_air_group('3')
            doctrine_xl3.gun_strafe_for_aircraft('1')
            # AGM-84L:816,AIM-120C-7:718,AIM-9M:1384
            edit_weapon_doctrine(doctrine=doctrine_xl3)
            airs_xl3 = {k: airs_dic[k] for k, v in airs_dic.items() if v.strName in lst_3}
            ...
            # patrol_mission.set_throttle_transit('Cruise')
            edit_thort_doctrine(patrol_mission)
            for air in airs_xl3.values():
                doctrine = air.get_doctrine()
                doctrine.set_emcon_according_to_superiors('no')
                doctrine.set_em_control_status('Radar', 'Passive')
        elif patrol_mission.strName == side_name + 'xl4':
            patrol_mission.set_prosecution_zone(ps_str_4)
            doctrine_xl4 = patrol_mission.get_doctrine()
            doctrine_xl4.set_weapon_state_for_aircraft(2002)
            doctrine_xl4.set_weapon_state_for_air_group('3')
            doctrine_xl4.gun_strafe_for_aircraft('1')
            # AGM-84L:816,AIM-120C-7:718,AIM-9M:1384
            edit_weapon_doctrine(doctrine=doctrine_xl4)
            airs_xl4 = {k: airs_dic[k] for k, v in airs_dic.items() if v.strName in lst_4}
...
def create_anti_ship_zone(side_name, scenario):
    ...
    for contact in contacts.values():
        if '护卫舰' in contact.strName:
            ...
            rp44 = side.add_reference_point(side_name + 'rp44', lat1+0.38, lon1-0.2)
            rp45 = side.add_reference_point(side_name + 'rp45', lat1+0.38, lon1-0.08)
            rp46 = side.add_reference_point(side_name + 'rp46', lat1+0.25, lon1-0.06)
            rp47 = side.add_reference_point(side_name + 'rp47', lat1+0.23, lon1-0.21)
...
def create_anti_ship_zone_2(side_name, scenario):
    ...
    for contact in contacts.values():
        if '护卫舰' in contact.strName:
            lat1, lon1 = contact.dLatitude, contact.dLongitude
            rp48 = side.add_reference_point(side_name + 'rp48', lat1+0.38, lon1-0.2)
            rp49 = side.add_reference_point(side_name + 'rp49', lat1+0.38, lon1-0.08)
            rp50 = side.add_reference_point(side_name + 'rp50', lat1+0.25, lon1-0.06)
            rp51 = side.add_reference_point(side_name + 'rp51', lat1+0.23, lon1-0.21)
...
def create_anti_ship_zone_3(side_name, scenario):
    ...
    for contact in contacts.values():
        if '护卫舰' in contact.strName:
            lat1, lon1 = contact.dLatitude, contact.dLongitude
            rp52 = side.add_reference_point(side_name + 'rp52', lat1+0.38, lon1-0.2)
            rp53 = side.add_reference_point(side_name + 'rp53', lat1+0.38, lon1-0.08)
            rp54 = side.add_reference_point(side_name + 'rp54', lat1+0.25, lon1-0.06)
            rp55 = side.add_reference_point(side_name + 'rp55', lat1+0.23, lon1-0.21)
            point_list.append([rp52, rp53, rp54, rp55])
    return point_list
# 更新反水面巡逻区
def update_anti_ship_zone(side_name, scenario):
    side = scenario.get_side_by_name(side_name)
    point_list = []
    contacts = side.contacts
    # 根据对方驱逐舰位置创建巡逻区 xl4
    for contact in contacts.values():
        if '驱逐舰' in contact.strName:
            lat1, lon1 = contact.dLatitude, contact.dLongitude
            rp44 = side.set_reference_point(side_name + 'rp44', lat1 + 0.26, lon1 - 0.6)
            rp45 = side.set_reference_point(side_name + 'rp45', lat1 + 0.26, lon1 - 0.4)
            rp46 = side.set_reference_point(side_name + 'rp46', lat1 + 0.1, lon1 - 0.5)
            rp47 = side.set_reference_point(side_name + 'rp47', lat1 + 0.09, lon1 - 0.6)
            point_list.append([rp44, rp45, rp46, rp47])
    return point_list
# 创建反水面警戒区
def create_anti_ship_prosecution_zone(side_name, scenario):
    ...
    for contact in contacts.values():
        if '护卫舰' in contact.strName:
            lat1, lon1 = contact.dLatitude, contact.dLongitude
            rp56 = side.add_reference_point(side_name + 'rp56', lat1+0.58, lon1-0.4)
            rp57 = side.add_reference_point(side_name + 'rp57', lat1+0.58, lon1-0.28)
            rp58 = side.add_reference_point(side_name + 'rp58', lat1+0.45, lon1-0.26)
            rp59 = side.add_reference_point(side_name + 'rp59', lat1+0.43, lon1-0.41)
...
def create_anti_ship_prosecution_zone_2(side_name, scenario):
    ...
    for contact in contacts.values():
        if '护卫舰' in contact.strName:
            lat1, lon1 = contact.dLatitude, contact.dLongitude
            rp60 = side.add_reference_point(side_name + 'rp60', lat1+0.58, lon1-0.4)
            rp61 = side.add_reference_point(side_name + 'rp61', lat1+0.58, lon1-0.28)
            rp62 = side.add_reference_point(side_name + 'rp62', lat1+0.45, lon1-0.26)
            rp63 = side.add_reference_point(side_name + 'rp63', lat1+0.43, lon1-0.41)
...
def create_anti_ship_prosecution_zone_3(side_name, scenario):
    ...
    for contact in contacts.values():
        if '护卫舰' in contact.strName:
            lat1, lon1 = contact.dLatitude, contact.dLongitude
            rp64 = side.add_reference_point(side_name + 'rp64', lat1+0.58, lon1-0.4)
            rp65 = side.add_reference_point(side_name + 'rp65', lat1+0.58, lon1-0.28)
            rp66 = side.add_reference_point(side_name + 'rp66', lat1+0.45, lon1-0.26)
            rp67 = side.add_reference_point(side_name + 'rp67', lat1+0.43, lon1-0.41)
