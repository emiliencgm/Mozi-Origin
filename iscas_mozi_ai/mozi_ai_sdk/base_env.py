...
class BaseEnvironment:
    ...
    def connect_mozi_server(self, ip=None, port=None):
        ...
        if ip is None and port is None:
            self.mozi_server = MoziServer(self.server_ip, self.aiPort,
                                          self.platform, self.scenario_name,
                                          self.simulate_compression,
                                          self.synchronous, self.platform_mode,
                                          self.agent_key_event_file)
        elif ip is not None and port is not None:
            self.mozi_server = MoziServer(ip, str(port), self.platform,
                                          self.scenario_name,
                                          self.simulate_compression,
                                          self.synchronous, self.platform_mode,
                                          self.agent_key_event_file)
