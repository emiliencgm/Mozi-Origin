...
def create_antisurfaceship_mission(side_name, scenario):
    ...
    airs_1 = {
        k: v
        for k, v in airs_dic.items() if int(re.sub('\D', '', v.strName)) < 3
    }
    # 闪电#3 #4
    airs_2 = {
        k: v
        for k, v in airs_dic.items()
        if 3 <= int(re.sub('\D', '', v.strName)) <= 4
    }
    ...
    targets = {
        ...
        if (('DDG' in v.strName) | ('CVN' in v.strName))
    }
...
def update_patrol_mission(side_name, scenario):
    ...
    airs = {
        k: v
        for k, v in airs_dic.items() if int(re.sub('\D', '', v.strName)) >= 5
    }
    ...
    for patrol_mission in patrol_missions:
        ...
        if patrol_mission.strName == side_name + 'xl3':
            ...
            # 如果飞机飞出，任务就打开
            airs3 = [
                v for v in airs_xl3.values() if '在空' in v.strActiveUnitStatus
            ]
            if len(airs3):
            ...
            list_str = [
                air.m_AssignedMission for air in air_other
                if '停放' in air.strActiveUnitStatus
            ]
            list_str = [i for i in list_str if i != '']
            set_str = set(list_str)
            air_sky = [
                air for air in air_other if '按计划' in air.strActiveUnitStatus
                or '参与' in air.strActiveUnitStatus
            ]
...
def evade_ship(geopoint_target, air, mission_doctrine):
    ...
    if geopoint_target:
        ...
        if dis <= 60:
            ...
            genpoint_away = get_end_point(geopoint_air, 15,
                                          (air.fCurrentHeading + 150))
...
def create_patrol_mission(side_name, scenario):
    ...
    if flag == 'Yes':
        return False
    airs_c = [
        v for v in airs_dic.values()
        if 4 < int(re.sub('\D', '', v.strName)) < 9
    ]  # 编号5，6,7,8的,4架飞机
    for air in airs_c:
        # 如果此4架飞机的挂载还是原来的挂载，那么修改挂载id 为 19364
        if 'AGM' in air.m_UnitWeapons:
            air.set_loadout('19364',
                            time_to_ready_minutes=1,
    ...
    for k, v in ships.items():
        ...
        if '驱逐舰' in v.strName:
            ...
            rp13 = side.add_reference_point(side_name + 'rp13', lat + 0.25,
                                            lon - 1)
            rp14 = side.add_reference_point(side_name + 'rp14', lat + 0.25,
                                            lon - 0.3)
            rp15 = side.add_reference_point(side_name + 'rp15', lat - 0.25,
                                            lon - 0.3)
            rp16 = side.add_reference_point(side_name + 'rp16', lat - 0.25,
                                            lon - 1)
