...
class TestSide(TestFramework):
    ...
    def test_get_identified_targets_by_name(self):
        """从推演方用名称确认目标"""
        contacts = self.red_side.get_identified_targets_by_name(
            '苏-34 型“鸭嘴兽”攻击机')
        for k, v in contacts.items():
    ...
    def test_get_elevation(self):
        """获取某点的海拔高度"""
        height = self.red_side.get_elevation(
            (self.airport_1.dLatitude, self.airport_1.dLongitude))
        self.assertEqual(self.airport_1.fCurrentAltitude_ASL, height)

    def test_add_unit(self):
        """添加单元"""
        self.red_side.add_unit('ship', 'B', 383, 61.4871723614997,
                               -17.2522930173048, 30)
        self.env.step()
        ships = self.red_side.get_ships()
        flag = False
    ...
    def test_add_submarine(self):
        """添加潜艇"""
        self.red_side.add_submarine('潜艇', 32, 61.4871723614997,
                                    -17.2522930173048, 30)
        self.env.step()
    ...
    def test_add_ship(self):
        """添加舰船"""
        self.red_side.add_ship('B', 383, 61.4871723614997, -17.2522930173048,
                               30)
        self.env.step()
        ships = self.red_side.get_ships()
        flag = False
    ...
    def test_add_facility(self):
        """添加地面兵力设施"""
        self.red_side.add_facility('地面兵力', 383, 30.9381715388612,
                                   121.062106870304, 30)
        self.env.step()
        facilities = self.red_side.get_facilities()
        flag = False
    ...
    def test_add_aircraft(self):
        """添加飞机"""
        self.red_side.add_aircraft('飞机-添加', 6, 119, 30.9381715388612,
                                   121.062106870304, 5000, 30)
        self.env.step()
    ...
    def test_add_unit_to_facility(self):
        """往机场，码头添加单元"""
        return_obj_1 = self.red_side.add_unit_to_facility(
            unit_type='air',
            name='基地飞机-新',
            dbid=13,
            base_unit_guid=self.airport_1_guid,
            loadout_id=8063)
        return_obj_2 = self.red_side.add_unit_to_facility(
            unit_type='ship',
            name='基地舰船-新',
            dbid=4,
            base_unit_guid=self.wharf_1_guid)
        return_obj_3 = self.red_side.add_unit_to_facility(
            unit_type='sub',
            name='基地潜艇-新',
            dbid=5,
            base_unit_guid=self.wharf_1_guid)
    ...
    def test_add_reference_point(self):
        """添加参考点"""
        point = self.red_side.add_reference_point('点1', 27.6642320262197,
                                                  134.621472742701)
        self.env.step()
    ...
    def test_add_zone(self):
        ...
        zone_1 = self.red_side.add_zone(zone_type=0,
                                        description='禁航区-新',
                                        area=area,
                                        affects=affects,
                                        is_active='true')
        zone_2 = self.red_side.add_zone(zone_type=1,
                                        description='封锁区-新',
                                        area=area,
                                        affects=affects,
                                        is_active='true',
                                        mark_as='Hostile')
        zone_3 = self.red_side.add_zone(zone_type=1,
                                        description='封锁区-新2',
                                        area=area,
                                        affects=affects,
                                        is_active='false',
    ...
    def test_set_zone(self):
        ...
        self.red_side.set_zone(zone_guid=zone_2.strGuid,
                               description='封锁区-该',
                               area=area,
                               affects=affects,
                               is_active='false',
    ...
    def test_assign_target_to_mission(self):
        ...
        self.red_side.assign_target_to_mission(self.enemy_airplane_guid,
                                               strike_mission.strGuid)
        self.red_side.assign_target_to_mission(self.enemy_airplane_guid_2,
                                               strike_mission.strName)
        self.env.step()
        self.env.step()
    ...
    def test_set_side_options(self):
        ...
        self.red_side.set_side_options(awareness=awareness,
                                       proficiency=proficiency,
                                       is_ai_only=is_ai_only,
                                       is_coll_response=is_coll_response,
                                       is_auto_track_civs=is_auto_track_civs)
        awareness = 'OMNI'
        proficiency = 'Ace'
        is_ai_only = 'false'
    ...
    def test_remove_zone(self):
        ...
        area = ['RP-66', 'RP-67', 'RP-68', 'RP-69']
        affects = ['Aircraft', 'Ship', 'Submarine']
        zone = self.red_side.add_zone(zone_type=1,
                                      description='封锁区-新',
                                      area=area,
                                      affects=affects,
                                      is_active='true',
    ...
    def test_delete_reference_point_by_name(self):
        """按参考点名称删除参考点"""
        point = self.red_side.add_reference_point('RP-XX', 27.6642320262197,
                                                  134.621472742701)
        self.env.step()
        self.env.step()
    ...
    def test_launch_units_in_group(self):
        """编组出航"""
        self.red_side.launch_units_in_group(
            [self.docked_ship_1, self.docked_ship_2])
        self.env.step()
    ...
    def test_launch_units_abort(self):
        ...
        self.red_side.launch_units_in_group(
            [self.docked_ship_1, self.docked_ship_2])
        self.env.step()
        self.red_side.launch_units_abort(
            [self.docked_ship_1, self.docked_ship_2])
        self.env.step()
