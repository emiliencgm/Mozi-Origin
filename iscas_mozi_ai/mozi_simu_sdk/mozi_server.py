...
class MoziServer:
        ...
        if self.platform == 'windows':
            ...
            for i in psutil.process_iter():
                if i.name() == 'MoziServer.exe':
                    str_tmp = str(i.name()) + "-" + str(i.pid) + "-" + str(
                        i.status())
                    print("墨子已启动")
    ...
    def connect_grpc_server(self):
        ...
        时间：4/2/20
        """
        conn = grpc.insecure_channel(self.server_ip + ':' +
    ...
    def load_scenario(self):
        ...
        for i in range(60):
            ...
            if str(value) == "'Yes'":
                ...
                load_success = True
                break
            print("%s：想定还没有加载完毕，再等一秒！可能原因，1）时间太短；2）服务端没有想定%s！" %
                  (datetime.datetime.now(), self.scenario_name))
            time.sleep(1)

        # 如果想定加载失败
        if not load_success:
            print("%s：超过50秒，想定没有加载成功。可能是服务端没有想定:%s！" %
                  (datetime.datetime.now(), scenario_file))
            return None
    ...
    def load_scenario_in_windows(self, scen_path, is_deduce):
        ...
        时间：4/2/20
        """
        return self.send_and_recv("Hs_ScenEdit_LoadScenario('{}', {})".format(
            scen_path, is_deduce))
    ...
    def load_scenario_in_linux(self, path, model):
        ...
        时间：4/2/20
        """
        return self.send_and_recv("Hs_PythonLoadScenario('{}', '{}')".format(
            path, model))
    ...
    def send_and_recv(self, cmd):
        ...
        if self.exect_flag:
            ...
            response = self.grpc_client.GrpcConnect(
                GRPCServerBase_pb2.GrpcRequest(name=cmd), timeout=30)
    ...
    def init_situation(self, scenario, app_mode):
        ...
        时间：4/9/20
        """
        b_init_success = scenario.situation.init_situation(
            self, scenario, app_mode)
    ...
    def set_key_value(self, key, value):
        ...
        时间：2020-3-11
        """
        return self.send_and_recv("ScenEdit_SetKeyValue('{}','{}')".format(
            key, value))
    ...
    def get_value_by_key(self, key):
        ...
        return self.send_and_recv(
            "ReturnObj(ScenEdit_GetKeyValue('{}'))".format(key))
