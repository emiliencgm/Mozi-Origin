...
class CSituation:
        ...
        if app_mode not in [2, 3]:
            ...
            for i in range(50):
                ...
                if load_result == "True":
                    ...
                    print("%s：想定已加载且打包完成！" % (datetime.now()))
                    load_success = True
                    break
                else:
                    # pylog.info("”")
                    print("%s：想定未加载或打包未完成，再等一秒！" % (datetime.now()))
                    time.sleep(1)

            if not load_success:
                print("%s：服务端超过50秒没有完成打包!" % (datetime.now()))
                return False

            # 取想定初始态势
            situation_str = mozi_server.send_and_recv("GetAllState")
            print("%s：接收到初始态势数据!" % (datetime.now()))
    ...
    def parse_init_situation(self, situation_str, scenario):
        ...
        :return:
        """
        print("%s：开始解析初始态势数据!" % (datetime.now()))
        try:
        ...
        print("%s：完成解析初始态势数据!" % (datetime.now()))
    ...
    def parse_contact(self, contact_json):
        ...
        if strGuid not in self.all_guid_info:
            ...
            self.all_guid_info[strGuid] = {
                "strType": 4001,
                "side": contact.m_OriginalDetectorSide
            }
            if self.update_start:
                self.all_guid_add_info[strGuid] = self.all_guid_info[strGuid]
            self.all_guid.append(strGuid)
    ...
    def parse_trgUnitDstrd(self, trgunitdstrd_json):
        ...
        if strGuid not in self.all_guid_info:
            trgunitdstrd = CTriggerUnitDestroyed(strGuid, self.mozi_server,
                                                 self)
    ...
    def parse_trgUnitRmnsInArea(self, trgunitrmns_json):
        ...
        if strGuid not in self.all_guid_info:
            trgunitrmns = CTriggerUnitRemainsInArea(strGuid, self.mozi_server,
                                                    self)
    ...
    def parse_cndScenHasStarted(self, cndscenhsstrtd_json):
        ...
        if strGuid not in self.all_guid_info:
            cndscenhsstrtd = CConditionScenHasStarted(strGuid,
                                                      self.mozi_server, self)
            ...
            self.cndscenhsstrtd_dic[strGuid].__dict__.update(
                cndscenhsstrtd_json)
    ...
    def parse_cndSidePosture(self, cndsidepstr_json):
        ...
        if strGuid not in self.all_guid_info:
            cndsidepstr = CConditionSidePosture(strGuid, self.mozi_server,
                                                self)
    ...
    def parse_actnTlptInArea(self, actiontlprt_json):
        ...
        if strGuid not in self.all_guid_info:
            actiontlprt = CActionTeleportInArea(strGuid, self.mozi_server,
                                                self)
    ...
    def parse_actnChngMssnStts(self, actionchngms_json):
        ...
        if strGuid not in self.all_guid_info:
            actionchngms = CActionChangeMissionStatus(strGuid,
                                                      self.mozi_server, self)
    ...
    def parse_delete(self, delete_json):
        ...
        if guid in self.all_guid_info:
            ...
            if strType == 1004:
                ...
                return
            if strType == 1005:
                self.all_guid_delete_info[guid] = {
                    "strType": 1005,
                    "side": self.group_dic[guid].m_Side
                }
            ...
            if strType == 2001:
                self.all_guid_delete_info[guid] = {
                    "strType": 2001,
                    "side": self.submarine_dic[guid].m_Side
                }
            ...
            if strType == 2002:
                self.all_guid_delete_info[guid] = {
                    "strType": 2002,
                    "side": self.ship_dic[guid].m_Side
                }
            ...
            if strType == 2003:
                self.all_guid_delete_info[guid] = {
                    "strType": 2003,
                    "side": self.facility_dic[guid].m_Side
                }
            ...
            if strType == 2004:
                self.all_guid_delete_info[guid] = {
                    "strType": 2004,
                    "side": self.aircraft_dic[guid].m_Side
                }
            ...
            if strType == 2005:
                self.all_guid_delete_info[guid] = {
                    "strType": 2005,
                    "side": self.satellite_dic[guid].m_Side
                }
            ...
            if strType == 3005:
                self.all_guid_delete_info[guid] = {
                    "strType": 3005,
                    "side": self.weapon_dic[guid].m_Side
                }
            ...
            if strType == 3006:
                self.all_guid_delete_info[guid] = {
                    "strType": 3006,
                    "side": self.unguidedwpn_dic[guid].m_Side
                }
            ...
            if strType == 3008:
                self.all_guid_delete_info[guid] = {
                    "strType": 3008,
                    "side": self.sideway_dic[guid].m_Side
                }
            ...
            if strType == 5001:
                self.all_guid_delete_info[guid] = {
                    "strType": 5001,
                    "side": self.logged_messages[guid].m_Side
                }
            ...
            if strType == 10001:
                self.all_guid_delete_info[guid] = {
                    "strType": 10001,
                    "side": self.mssnpatrol_dic[guid].m_Side
                }
            ...
            if strType == 10002:
                self.all_guid_delete_info[guid] = {
                    "strType": 10002,
                    "side": self.mssnstrike_dic[guid].m_Side
                }
            ...
            if strType == 10003:
                self.all_guid_delete_info[guid] = {
                    "strType": 10003,
                    "side": self.mssnsupport_dic[guid].m_Side
                }
            ...
            if strType == 10004:
                self.all_guid_delete_info[guid] = {
                    "strType": 10004,
                    "side": self.mssncargo_dic[guid].m_Side
                }
            ...
            if strType == 10005:
                self.all_guid_delete_info[guid] = {
                    "strType": 10005,
                    "side": self.mssnferry_dic[guid].m_Side
                }
            ...
            if strType == 10006:
                self.all_guid_delete_info[guid] = {
                    "strType": 10006,
                    "side": self.mssnmining_dic[guid].m_Side
                }
            ...
            if strType == 10007:
                self.all_guid_delete_info[guid] = {
                    "strType": 10007,
                    "side": self.mssnmnclrng_dic[guid].m_Side
                }
            ...
            if strType == 11001:
                self.all_guid_delete_info[guid] = {
                    "strType": 11001,
                    "side": self.referencept_dic[guid].m_Side
                }
            ...
            if strType == 11002:
                self.all_guid_delete_info[guid] = {
                    "strType": 11002,
                    "side": self.zonenonav_dic[guid].m_Side
                }
            ...
            if strType == 11003:
                self.all_guid_delete_info[guid] = {
                    "strType": 11003,
                    "side": self.zonexclsn_dic[guid].m_Side
                }
        ...
        while continuing:
            for i in range(1, 37):
                if i in hyphen_order:
                    ...
                    letter = chr(random.randint(
                        97, 102))  # 102-f (MoziServer), 122-z (wider)
                    s.extend(str(random.choice([num, letter])))
            if (s not in self.all_guid) and (s
                                             not in self.pseudo_situ_all_guid):
                continuing = False
        return ''.join(s)
