...
from mozi_ai_sdk.feihai_blue_aircraft_dueling_dqn.envs.env_feihai_blue_aircraft_dqn import feihaiblue as Environment_aircraft_dqn
# from mozi_ai_sdk.feihai_blue_ship_dueling_dqn.envs import etc
from mozi_ai_sdk.feihai_blue_aircraft_dueling_dqn.envs import etc
# from mozi_ai_sdk.feihai_blue_ship_dueling_dqn.Dueling_DQN import DuelingDQN
from mozi_ai_sdk.feihai_blue_aircraft_dueling_dqn.Dueling_DQN import DuelingDQN
# from mozi_ai_sdk.feihai_blue_ship_dueling_dqn.utils import plot_learning_curve, create_directory
from mozi_ai_sdk.feihai_blue_aircraft_dueling_dqn.utils import plot_learning_curve, create_directory
# 获取IP和Port
arg = sys.argv
print(arg)
...
def create_env():
    print('开发模式')
    env = Environment_aircraft_dqn(etc.SERVER_IP, etc.SERVER_PORT, None, etc.DURATION_INTERVAL, etc.app_mode,
...
def create_dqn_agent(env):
    agent = DuelingDQN(alpha=0.0003, state_dim=env.observation_space, action_dim=env.action_space,
...
def train():
    ...
    log_f.write('episode,total_reward,reward\n')
    try:
        ...
        for episode in range(FLAGS.max_episodes):
            ...
            log_f.write('{},{},{}\n'.format(episode + 1, round(total_reward, 1), round(avg_reward, 1)))
            log_f.flush()
