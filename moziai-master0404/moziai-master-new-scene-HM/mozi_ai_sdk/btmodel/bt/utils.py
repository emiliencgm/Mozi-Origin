...
def create_patrol_zone(side, pos):
    ...
    # lat: 纬度， lon：经度
    rp1 = side.add_reference_point('strike_rp1', lat + 0.13, lon - 0.13)
    rp2 = side.add_reference_point('strike_rp2', lat + 0.13, lon + 0.13)
    rp3 = side.add_reference_point('strike_rp3', lat - 0.13, lon + 0.13)
    rp4 = side.add_reference_point('strike_rp4', lat - 0.13, lon - 0.13)
    point_list = [rp1, rp2, rp3, rp4]

    return point_list

# 仿照create_patrol_zone这个函数重新写个生成巡逻区的
# 航母位置latitude='18.7656164194389', longitude='124.579126241413'
# 其余四点位置latitude='19.2124840546196', longitude='124.118878849577'
# latitude='18.20262373074', longitude='124.095785287948'
# latitude='18.1708308921269', longitude='124.838777662086'
# latitude='19.2432619236264', longitude='124.890788326922'
def create_strike_patrol_zone(side, pos):
    lat = pos['latitude']
    lon = pos['longitude']
    # p1 = (lat - 0.10, lon - 0.13)
    # lat: 纬度， lon：经度
    rp5 = side.add_reference_point('strike_rp5', lat + 0.23, lon - 0.23)
    rp6 = side.add_reference_point('strike_rp6', lat + 0.23, lon + 0.23)
    rp7 = side.add_reference_point('strike_rp7', lat - 0.23, lon + 0.23)
    rp8 = side.add_reference_point('strike_rp8', lat - 0.23, lon - 0.23)
    point_list = [rp5, rp6, rp7, rp8]
