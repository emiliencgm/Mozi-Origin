...
from mozi_ai_sdk.zhou.env.env import Environment
from mozi_ai_sdk.zhou.env import etc
from mozi_ai_sdk.zhou.leaf_nodes_eg import *
import argparse
import sys
import os
import traceback
...
def run(env, side_name=None):
    ...
    # 实例化智能体
    step_count = 0
    all_first_group_list = get_group_first(side_name, env.scenario)
    # send_strike_order(side_name, env.scenario)
    # set_group_attribute(side_name, env.scenario, all_first_group_list)
    while True:
        env.step()
        # 更新动作
        # if step_count > 100:
        #     # all_first_group_list = get_group_first(side_name, env.scenario)
        #     set_group_attribute(side_name, env.scenario, all_first_group_list)
        add_support_mission(side_name, env.scenario)
        send_strike_order(side_name, env.scenario)
        set_group_attribute(side_name, env.scenario, all_first_group_list)
