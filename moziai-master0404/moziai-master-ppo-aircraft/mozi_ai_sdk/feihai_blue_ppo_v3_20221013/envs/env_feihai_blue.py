...
from mozi_ai_sdk.feihai_blue_ppo_v3_20221013.envs.utils import *
...
class feihaiblue(BaseEnvironment):
    ...
    def get_reward(self, action_for_final_patrol_zone):
        ...
        if action_for_final_patrol_zone is not None:
            ...
            score_reward = self._get_score_reward()

            # 获得距离得分，蓝方一架飞机距离格子中心点小于80公里，则加1分，否则减1分
    ...
    def _get_num_reward(self, final_zone):
        ...
        for aircraft in self.red_unit_dict.values():
            dis = get_horizontal_distance((aircraft.dLatitude, aircraft.dLongitude), (central_lat, central_lon))
            dis_list.append(dis)
        dis_nearest = [True for d in dis_list if d < 70]
        if len(dis_nearest) != 0:
            num_reward = len(dis_nearest) / 10
        return num_reward

    def _get_score_reward(self):
        score_reward = 0
        score = float(self.blueside.iTotalScore)
        if score > 180:
            score_reward = 1
        return score_reward
    def _get_distance_reward(self, final_zone):
        # 由于蓝方飞机是集体行动的，所以从10架里距离格子中心的最大距离小于60，加1分，否则减1分
        ...
        dis_list = []
        # random_aircraft = random.choice(list(self.blue_unit_dict.values()))
        for aircraft in self.blue_unit_dict.values():
            dis = get_horizontal_distance((aircraft.dLatitude, aircraft.dLongitude),(central_lat, central_lon))
            dis_list.append(dis)
