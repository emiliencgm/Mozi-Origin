...
import random
import numpy as np
from mozi_ai_sdk.feihai_blue_ppo_ship_discrete_v2.envs.utils import *
...
class feihaiblue_ship_ppo_discrete(BaseEnvironment):
    ...
    def get_reward_ship(self, action_for_final_patrol_zone):
        ...
        if action_for_final_patrol_zone is not None:
            ...
            num_reward = self._get_num_reward_ship(action_for_final_patrol_zone) / 3
            # 获取得分奖励，战损战果
    ...
    def _get_num_reward_ship(self, final_zone):
        ...
        num = 0
        for k, v in self.red_unit_dict_ship.items():
            unit = {}
    ...
    def _get_distance_reward_ship(self, final_zone):
        ...
        dis_reward = 0
        random_aircraft = random.choice(list(self.blue_unit_dict_ship.values()))
        # 获取格子中心点坐标
        central_lat = (final_zone[0][0] + final_zone[2][0]) / 2
        central_lon = (final_zone[0][1] + final_zone[1][1]) / 2
    ...
    def check_target_alive(self):
        ...
        target_huwei = [value for key, value in self.red_unit_dict_ship.items() if '护卫舰' in value.strName]
        # 如果船失火或者进水了，就加分
        if target_quzhu[0].iFireIntensityLevel == 0 or target_quzhu[0].iFloodingIntensityLevel == 0:
            reward_alive += 1
        elif target_huwei[0].iFireIntensityLevel == 0 or target_huwei[0].iFloodingIntensityLevel == 0:
            reward_alive += 1
        return reward_alive
