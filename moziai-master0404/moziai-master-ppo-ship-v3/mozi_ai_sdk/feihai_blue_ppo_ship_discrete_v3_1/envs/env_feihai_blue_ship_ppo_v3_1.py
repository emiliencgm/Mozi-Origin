...
import random
import numpy as np
from mozi_ai_sdk.feihai_blue_ppo_ship_discrete_v3_1.envs.utils import *
...
功能：打船的环境文件,反水面战巡逻
'''
class feihaiblue_ship_ppo_discrete(BaseEnvironment):
    """
    作者：
    日期：
    功能：构造函数
    ...
    def __init__(self, IP, AIPort, agent_key_event_file, duration_interval, app_mode, synchronous=None,
        ...
        self.observation_space_ship = 33  # 状态空间维度
        self.action_space_ship = 9 # 9个离散动作，对应打船的9个区域
        self.blue_unit_dict_ship = None
        self.red_unit_dict_ship = None
        self.observation_ship = None
        ...
        self.train_zones_ship = None
    def init_ship_train_zone(self):
        # 初始化9宫格
        # 对于打船
        # blueside = self.scenario.get_side_by_name(self.blue_side_name)
        # latitude = '19.755054723756', longitude = '123.47380654949'
        # latitude = '19.7528855276267', longitude = '126.723546109339'
        # latitude = '18.0675196521575', longitude = '126.692868812231'
        # latitude = '18.0839662840567', longitude = '123.476104752405'
        # latitude = '21.0091063565163', longitude = '122.215210323807'
        # latitude = '20.9929933780928', longitude = '127.990025011534'
        # latitude = '16.4180334551013', longitude = '127.915620573564'
        # latitude = '16.4191455295843', longitude = '122.295255310997'

        # start_lat = 19.755054723756
        # start_lon = 123.47380654949
        # end_lat = 18.0675196521575
        # end_lon = 126.692868812231
        start_lat = 21.0091063565163
        start_lon = 122.215210323807
        end_lat = 16.4180334551013
        end_lon = 127.990025011534
        ...
        for i in np.linspace(end_lat, start_lat, 4):
            for j in np.linspace(start_lon, end_lon, 4):
                a.append([i, j])
                k += 1
                self.blueside.add_reference_point('dqn' + str(k), i, j)
    ...
    # 先创建一个反水面战巡逻任务，让打船飞机动起来
    def init_creat_patrol_ship_mission(self):
        flag = self.scenario.mozi_server.get_value_by_key(f'{self.blue_side_name}反水面巡逻任务已创建')
        if flag == 'Yes':
            return True
        nine_zone = self.train_zones_ship
        # 从9个区域里随机找一个，作为初始巡逻区
        patrol_zone_start = random.choice(nine_zone)
        point_list = self.create_patrol_ship_zone(self.blueside, patrol_zone_start)
        i = 1
        point_prosecution_list = self.creat_prosecution_ship_area(self.blueside)
        ps_str = []
        for ps in point_prosecution_list:
            for name in ps:
                ps_str.append(name.strName)
        for point in point_list:
            ...
            for name in point:
                point_str.append(name.strName)
            # 新建巡逻区名字
            patrol_name = self.blue_side_name + 'xl_ship' + str(i)
            patrolmssn = self.blueside.add_mission_patrol(patrol_name, 1, point_str)
            print('反水面巡逻任务已创建')
            patrolmssn.set_start_time('2021-05-26 12:00:00')
            patrolmssn.set_one_third_rule('false')
            patrolmssn.set_patrol_zone(point_str)
            patrolmssn.set_prosecution_zone(ps_str)
            airs_xl_ship = self.blue_unit_dict_ship
            patrolmssn.assign_units(airs_xl_ship)
            patrolmssn.set_flight_size(1)
            patrolmssn.set_flight_size_check('false')
            patrolmssn.set_opa_check('true')
            patrolmssn.set_wwr_check('true')
        ...
        self.scenario.mozi_server.set_key_value(f'{self.blue_side_name}反水面巡逻任务已创建', 'Yes')
        return False
    # 创建反水面战巡逻任务的初始巡逻区，随机选的，后续根据选择的动作更新
    def create_patrol_ship_zone(self, side, point_1):
        point_list = []
        rp1_ship = side.add_reference_point(self.blue_side_name + 'xl_ship_point1', point_1[0][0], point_1[0][1])
        rp2_ship = side.add_reference_point(self.blue_side_name + 'xl_ship_point2', point_1[1][0], point_1[1][1])
        rp3_ship = side.add_reference_point(self.blue_side_name + 'xl_ship_point3', point_1[3][0], point_1[3][1])
        rp4_ship = side.add_reference_point(self.blue_side_name + 'xl_ship_point4', point_1[2][0], point_1[2][1])
        point_list.append([rp1_ship, rp2_ship, rp3_ship, rp4_ship])
        return point_list
    # 创建反水面战巡逻的警戒区，初始是一个大区域，后续根据选择的动作更新
    def creat_prosecution_ship_area(self, side):
        point_list = []
        # latitude = '19.755054723756', longitude = '123.47380654949'
        # latitude = '19.7528855276267', longitude = '126.723546109339'
        # latitude = '18.0675196521575', longitude = '126.692868812231'
        # latitude = '18.0839662840567', longitude = '123.476104752405'
        point_1 = [(19.755054723756, 123.47380654949), (19.7528855276267, 126.723546109339),
                   (18.0675196521575, 126.692868812231), (18.0839662840567, 123.476104752405)]
        rp5_ship = side.add_reference_point(self.blue_side_name + 'jj_ship_5', point_1[0][0], point_1[0][1])
        rp6_ship = side.add_reference_point(self.blue_side_name + 'jj_ship_6', point_1[1][0], point_1[1][1])
        rp7_ship = side.add_reference_point(self.blue_side_name + 'jj_ship_7', point_1[2][0], point_1[2][1])
        rp8_ship = side.add_reference_point(self.blue_side_name + 'jj_ship_8', point_1[3][0], point_1[3][1])
        point_list.append([rp5_ship, rp6_ship, rp7_ship, rp8_ship])
        return point_list
    # 用固定区域作为初始支援任务的支援区
    ...
    def edit_anti_ship_weapon_doctrine(self, doctrine):
        # AGM - 84L: 816
        doctrine.set_weapon_release_authority('816', '2999', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3101', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3102', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3103', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3000', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3104', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3105', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3106', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3107', '2', '1', '40', 'none', 'false')
        doctrine.set_weapon_release_authority('816', '3108', '2', '1', '40', 'none', 'false')
    ...
    def _init_unit_list(self):
        ...
        """
        self.blue_unit_dict_ship = self._init_blue_unit_dict_ship()
        self.red_unit_dict_ship = self._init_red_unit_dict_ship()
    def _init_blue_unit_dict_ship(self):
        """
    ...
    def _init_red_unit_dict_ship(self):
        """
        初始化红方单元列表
        """
    ...
    def get_observation_ship(self):
        """
        获取红蓝双方的经纬度、朝向，作为模型的state，即输入到模型，
        """
        blue_aircraft_unit_dict = self.blue_unit_dict_ship
        blue_obs = self.get_blue_side_observation_ship(blue_aircraft_unit_dict)
        red_ship_unit_dict = self.red_unit_dict_ship
        red_obs = self.get_red_side_observation_ship(red_ship_unit_dict)
        all_obs_array = np.array(blue_obs + red_obs)
        self.observation = all_obs_array
        return all_obs_array
    def get_blue_side_observation_ship(self,unit_dict):
        blue_obs_lt = []
        ...
        return blue_obs_lt
    def get_red_side_observation_ship(self,unit_dict):
        red_obs_lt = []
        for key, unit in unit_dict.items():
            if key:
    ...
    流程： 
        输入动作：9个离散区域
        执行动作：给打船飞机设置反水面战巡逻任务，动作对应9宫格
        更新态势
        获取观察
        获取reward
        检查是否结束
        如果最后收敛的情况下，会选择一个最好的区域     
    参数：无
    返回： 1）state：状态；
           2）reward：回报值
    '''
    def execute_action(self, action):
        super(feihaiblue_ship_ppo_discrete, self).step()
        # 首先下达一个对海打击任务，让飞机飞出去，然后利用学习出来的偏差值，设置航路点
        nine_zone = self.train_zones_ship
        print('action=',action)
        final_patrol_zone = nine_zone[action]
        # 先初始化一个反水面巡逻任务，让打船飞机动起来
        flag = self.init_creat_patrol_ship_mission()
        if flag:
            # 如果已经有打击任务了，就不在创建了，直接利用学习到action设置巡逻区
            self.update_patrol_ship_mission_zone(self.blueside, final_patrol_zone)
            self.update_presection_ship_zone(self.blueside, final_patrol_zone)
        ...
        obs = self.get_observation_ship()
        reward = self.get_reward_ship(final_patrol_zone)
        done = self.check_done_ship()

        return np.array(obs), reward, done

    def update_support_zone(self,side):
        # 预警机、干扰机前推
        target_huwei = [value for key, value in self.red_unit_dict_ship.items() if '护卫舰' in value.strName]
        if len(target_huwei) != 0:
            geopoint_target = (target_huwei[0].dLatitude, target_huwei[0].dLongitude)
            for air in self.blue_unit_dict_ship.values():
    ...
    def update_patrol_ship_mission_zone(self,side, final_zone):
        # 利用学习到动作值，更新巡逻区
        # 利用学习到动作值，找到相应区域，然后利用区域中心，生成小范围巡逻区
        # central_lat = (final_zone[0][0] + final_zone[2][0]) / 2
        # central_lon = (final_zone[0][1] + final_zone[1][1]) / 2

        # side.set_reference_point(self.blue_side_name + 'xl_ship_point1', central_lat + 0.03, central_lon - 0.05)
        # side.set_reference_point(self.blue_side_name + 'xl_ship_point2', central_lat + 0.04, central_lon + 0.06)
        # side.set_reference_point(self.blue_side_name + 'xl_ship_point3', central_lat - 0.04, central_lon + 0.05)
        # side.set_reference_point(self.blue_side_name + 'xl_ship_point4', central_lat - 0.03, central_lon - 0.06)
        side.set_reference_point(self.blue_side_name + 'xl_ship_point1', final_zone[0][0], final_zone[0][1])
        side.set_reference_point(self.blue_side_name + 'xl_ship_point2', final_zone[1][0], final_zone[1][1])
        side.set_reference_point(self.blue_side_name + 'xl_ship_point3', final_zone[3][0], final_zone[3][1])
        side.set_reference_point(self.blue_side_name + 'xl_ship_point4', final_zone[2][0], final_zone[2][1])

    def update_presection_ship_zone(self,side, final_zone):
        # 利用学习到动作值，更新警戒区,对应打船先不用
        side.set_reference_point(self.blue_side_name + 'jj_ship_5', final_zone[0][0], final_zone[0][1])
        side.set_reference_point(self.blue_side_name + 'jj_ship_6', final_zone[1][0], final_zone[1][1])
        side.set_reference_point(self.blue_side_name + 'jj_ship_7', final_zone[3][0], final_zone[3][1])
        side.set_reference_point(self.blue_side_name + 'jj_ship_8', final_zone[2][0], final_zone[2][1])
    ...
    def get_reward_ship(self, action_for_final_patrol_zone):
        """
        ...
        if action_for_final_patrol_zone is not None:
            # 在某个格子内，包括红方舰船，给与正奖励
            # 这是中间奖励
            num_reward = self._get_num_reward_ship(action_for_final_patrol_zone)
            print('num_reward=', num_reward)
            # 获取得分奖励，战损战果，# 这是最终目标
            score_reward = self.check_target_alive()
            print('score_reward=',score_reward)
            # 获得距离得分，蓝方一架飞机距离格子中心点小于60公里，则加1分，否则减1分 ，# 这是中间奖励
            distance_reward = self._get_distance_reward_ship(action_for_final_patrol_zone)
            # 对三个reward 进行加权求和作为总reward
            print('distance_reward=', distance_reward)
            reward = 0.1 * num_reward + 0.8 * score_reward + 0.1 * distance_reward
        return reward

    def _get_num_reward_ship(self, final_zone):
        # 获得该区域内船数量越多，reward越大
        central_lat = (final_zone[0][0] + final_zone[2][0]) / 2
        central_lon = (final_zone[0][1] + final_zone[1][1]) / 2
        dis_list = []
        num_reward = -1
        # random_aircraft = random.choice(list(self.blue_unit_dict.values()))
        for ship in self.red_unit_dict_ship.values():
            dis = get_horizontal_distance((ship.dLatitude, ship.dLongitude), (central_lat, central_lon))
            dis_list.append(dis)
        dis_nearest = [True for d in dis_list if d < 80]
        if len(dis_nearest) != 0:
            num_reward = len(dis_nearest) / 3
        return num_reward
    def _get_distance_reward_ship(self, final_zone):
        # 由于蓝方飞机是集体行动的，所以从8架里随机选一架，判断和格子中心点的距离，小于60，加1分，否则减1分
        ...
        dis_list = []
        # random_aircraft = random.choice(list(self.blue_unit_dict.values()))
        for aircraft in self.blue_unit_dict_ship.values():
            dis = get_horizontal_distance((aircraft.dLatitude, aircraft.dLongitude), (central_lat, central_lon))
            dis_list.append(dis)
    ...
    def check_target_alive(self):
        reward_alive = 0
        target_quzhu = [value for key, value in self.red_unit_dict_ship.items() if '驱逐舰' in value.strName]
        target_huwei = [value for key, value in self.red_unit_dict_ship.items() if '护卫舰' in value.strName]
        # 如果船失火或者进水了，就加分
        if target_quzhu[0].iFireIntensityLevel in [1, 2, 3, 4] or target_quzhu[0].iFloodingIntensityLevel in [1, 2, 3, 4]:
            reward_alive += 1
        if target_huwei[0].iFireIntensityLevel in [1, 2, 3, 4] or target_huwei[0].iFloodingIntensityLevel in [1, 2, 3, 4]:
            reward_alive += 1
        return reward_alive

    def check_done_ship(self):
