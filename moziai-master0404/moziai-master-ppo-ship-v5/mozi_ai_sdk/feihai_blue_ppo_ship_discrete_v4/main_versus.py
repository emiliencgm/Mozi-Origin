...
from mozi_ai_sdk.feihai_blue_ppo_ship_discrete_v4.envs.env_feihai_blue_ship_ppo_v4 import feihaiblue_ship_ppo_discrete as Environment_ship_discrete
# from mozi_ai_sdk.test.dppo.envs import env_remote as environment
from mozi_ai_sdk.feihai_blue_ppo_ship_discrete_v4.envs import etc
from mozi_ai_sdk.dppo.utils.utils import print_arguments
from mozi_ai_sdk.feihai_blue_ppo_ship_discrete_v4.PPO import PPO
...
def create_env():
    ...
    if FLAGS.platform_mode == 'versus':
        ...
        ip = FLAGS.IP
        port = FLAGS.Port
        env = Environment_ship_discrete(ip, port, etc.PLATFORM, etc.SCENARIO_NAME, etc.SIMULATE_COMPRESSION, etc.DURATION_INTERVAL,
                          etc.SYNCHRONOUS)
        ...
        env = Environment_ship_discrete(etc.SERVER_IP, etc.SERVER_PORT, None, etc.DURATION_INTERVAL, etc.app_mode,
...
def create_ppo_agent(env):
    ...
    # env.action_space = 41
    # env.action_space = 56
    agent = PPO(env.observation_space_ship, env.action_space_ship, lr_actor, lr_critic, gamma, K_epochs, eps_clip, has_continuous_action_space, action_std)
    return agent
...
def train():
    ...
    try:
        ...
        max_ep_len = 1000  # max timesteps in one episode,Episode长度通常取4096, 2048, 1024等2的次幂
