...
lst_1 = ['Udoshi #1', 'Udoshi #2', 'Udoshi #3', 'Udoshi #4', 'Udoshi #5', 'Udoshi #6', 'Udoshi #7', 'Udoshi #8',
         'Udoshi #9', 'Udoshi #10']
# lst_2
lst_2 = ['Udoshi #11', 'Udoshi #12', 'Udoshi #13', 'Udoshi #14', 'Udoshi #15', 'Udoshi #16', 'Udoshi #17', 'Udoshi #18',
         'Udoshi #19', 'Udoshi #20']
...
lst_3 = ['Udoshi #21', 'Udoshi #22', 'Udoshi #23', 'Udoshi #24', 'Udoshi #25', 'Udoshi #26', 'Udoshi #27', 'Udoshi #28',
         'Udoshi #29', 'Udoshi #30']
lst_4 = ['Udoshi #31', 'Udoshi #32', 'Udoshi #33', 'Udoshi #34', 'Udoshi #35', 'Udoshi #36', 'Udoshi #37']
# lst_5 固定区域空中巡逻
lst_5 = ['EC-130H #2']
...
def update_antisurfaceship_mission(side_name, scenario):
    ...
    edit_strike_doctrine(strike_doctrine=doctrine)

    doctrine_2 = strkmssn_2.get_doctrine()
    # doctrine.set_emcon_according_to_superiors('false')
    edit_strike_doctrine(strike_doctrine=doctrine_2)
    ...
    strkmssn4 = [v for v in mssnSitu.values() if 'strike4' in v.strName]
    missionUnits = patrol_missions_two.m_AssignedUnits.split('@')
    missionUnits_fight = [unit for unit in missionUnits if
                          airs_dic[unit].strName != 'E-2K #1' and airs_dic[unit].strName != 'EC-130H #1']
    side_red = scenario.get_side_by_name('红方')
    airs = side_red.aircrafts
    airs_fight = [v for v in airs.values() if '米格' in v.strName]
    if len(airs_fight) < 5 and len(strkmssn4) == 0:
        print('把巡逻2任务换成对舰打击')
        strkmssn_4 = side.add_mission_strike('strike4', 2)
        strkmssn_4.set_flight_size_check('false')
        for k, v in target.items():
            strkmssn_4.assign_unit_as_target(k)
        side.add_plan_way(0, 'strike4Way')

        update_patrol_zone(side_name, scenario)
        pos = {'latitude': list(target.values())[0].dLatitude+0.3, 'longitude': list(target.values())[0].dLongitude-0.2}
        pos1 = {'latitude': list(target.values())[0].dLatitude-0.1, 'longitude': list(target.values())[0].dLongitude-0.1}
        pos2 = {'latitude': list(target.values())[0].dLatitude + 0.1,
               'longitude': list(target.values())[0].dLongitude + 0.2}
        wayPointList4 = [pos,pos1,pos2]
        for item in wayPointList4:
            side.add_plan_way_point('strike4Way', item['longitude'], item['latitude'])
        strkmssn_4.add_plan_way_to_mission(0, 'strike4Way')
        utils.change_unit_mission(side, patrol_missions_two, strkmssn_4, missionUnits_fight)
        return False
    return False
...
def update_patrol_mission(side_name, scenario):
    ...
    for patrol_mission in patrol_missions:
        if patrol_mission.strName == side_name + 'xl1':
            ...
            # lst_5 执行xl1,定点干扰和预警
            airs_xl1 = {k: airs_dic[k] for k, v in airs_dic.items() if v.strName in lst_5}
            ...
            for air in airs_xl1.values():
                if air:
                    ...
                    if air.strName == 'EC-130H #2' or air.strName == 'EC-130H #1':
                        doctrine = air.get_doctrine()
                        doctrine.set_em_control_status('OECM', 'Active')
                        air.plot_course([(xl1_lat-0.04, xl1_lon-0.9)])
                    elif air.strName == 'E-2K #1':
                        doctrine = air.get_doctrine()
                        doctrine.set_em_control_status('Radar', 'Active')
                        air.plot_course([(xl1_lat - 0.04, xl1_lon - 0.9)])
    mssnSitu = side.strikemssns
    strkmssn_3 = [v for v in mssnSitu.values() if v.strName == 'strike3'][0]
    doctrine3 = strkmssn_3.get_doctrine()
    edit_strike_doctrine(strike_doctrine=doctrine3)
    edit_weapon_doctrine(doctrine=doctrine3)
    return False
...
def create_patrol_mission(side_name, scenario):
    ...
    side = scenario.get_side_by_name(side_name)
    airs_dic = side.aircrafts
    airs_dj = {k: airs_dic[k] for k, v in airs_dic.items() if v.strName in lst_1}
    contacts = side.contacts
    patrol_missions_dic = side.get_patrol_missions()
    ...
    for point in point_list:
        ...
        # i += 1
    # 创建空中拦截任务
    targets = {k: v for k, v in contacts.items() if (('米格' in v.strName) | ('卡-29' in v.strName))}
    target_1 = {k: v for k, v in contacts.items() if ('米格' in v.strName)}
    for k, v in targets.items():
        # set_mark_contact设置目标对抗关系,H is 敌方
        side.set_mark_contact(k, 'H')
    mssnSitu = side.strikemssns
    ...
    strkmssn_3.assign_units(airs_dj)
    side.add_plan_way(0, 'air_strike_way')
    # latitude = '20.9679758630618', longitude = '122.658783119652'
    # latitude = '20.1989454799695', longitude = '123.51091580901'
    # latitude = '19.5825286887569', longitude = '124.052179057153'
    air_strike_way_point_list = [{'latitude': '20.9679758630618', 'longitude': '122.658783119652'},
                     {'latitude': '20.1989454799695', 'longitude': '123.51091580901'},
                     {'latitude': '19.5825286887569', 'longitude': '124.052179057153'}]
    for item in air_strike_way_point_list:
        side.add_plan_way_point('air_strike_way', item['longitude'], item['latitude'])
    strkmssn_3.add_plan_way_to_mission(0, 'air_strike_way')
...
def create_patrol_zone_1(side_name, scenario):
    side = scenario.get_side_by_name(side_name)
    point_list = []
    # latitude = '18.9089276835998', longitude = '124.963099315572'
    # latitude = '18.4797552046523', longitude = '125.461991653289'
    # latitude = '18.4440149144049', longitude = '124.773565836168'
    # latitude = '18.2105565910211', longitude = '125.326506672064'
    # point_1 = [(18.9089276835998, 124.963099315572), (18.4797552046523, 125.461991653289),
    #        (18.4440149144049, 124.773565836168), (18.2105565910211, 125.326506672064)]
    point_1 = [(21.8210656716398, 122.356667127581), (21.8752434450009, 122.768058783733),
               (21.5245647261449, 122.005001630342), (21.6236431494821, 123.354562698353)]
    '''
    这四个点是手动设置的巡逻区，小范围,名字是RP-3756,RP-3760,RP-3761,RP-3762,先试试这四个点
    latitude = '21.8210656716398', longitude = '122.356667127581'
    latitude = '21.8752434450009', longitude = '122.768058783733'
...
# 设置反海巡逻的警戒区的区域
def creat_strike_surface_prosecution_area(side_name, scenario):
    side = scenario.get_side_by_name(side_name)
    point_list = []
    # latitude = '19.2879761711419', longitude = '124.073280457511'
    # latitude = '18.5607346736615', longitude = '125.827247031015'
    # latitude = '18.6694619229627', longitude = '123.724826177693'
    # latitude = '18.0289934085409', longitude = '125.39393676684'
    point_1 = [(19.2879761711419, 124.073280457511), (18.5607346736615, 125.827247031015), (18.6694619229627, 123.724826177693),
               (18.0289934085409, 125.39393676684)]
    rp13 = side.add_reference_point(side_name + 'rp13', point_1[0][0], point_1[0][1])
    rp14 = side.add_reference_point(side_name + 'rp14', point_1[1][0], point_1[1][1])
    rp15 = side.add_reference_point(side_name + 'rp15', point_1[2][0], point_1[2][1])
    rp16 = side.add_reference_point(side_name + 'rp16', point_1[3][0], point_1[3][1])
...
# 设置打击任务条例
def edit_strike_doctrine(strike_doctrine):
    if strike_doctrine.m_WeaponStateRTB != 3:
        strike_doctrine.set_weapon_state_for_air_group('3')  # m_WeaponStateRTB
    if strike_doctrine.m_GunStrafeGroundTargets != 1:
        strike_doctrine.gun_strafe_for_aircraft('1')  # m_GunStrafeGroundTargets
    if strike_doctrine.m_BingoJokerRTB != 0:
        strike_doctrine.set_fuel_state_for_air_group('3')  # m_BingoJokerRTB
        # 0， 对海目标自由开火
    if strike_doctrine.m_WCS_Surface != 0:
        strike_doctrine.set_weapon_control_status('weapon_control_status_surface', '0')
        # 0， 对空目标自由开火
    if strike_doctrine.m_WCS_Air != 0:
        strike_doctrine.set_weapon_control_status('weapon_control_status_air', '0')
